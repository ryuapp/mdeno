...
impl<'js> UrlSearchParams<'js> {
    ...
    pub fn new_with_url(_ctx: Ctx<'js>, url: Rc<RefCell<ada_url::Url>>) -> rquickjs::Result<Self> {
        ...
        let inner = ada_url::UrlSearchParams::parse(query).map_err(|e| {
            eprintln!(
                "[URLSearchParams.new_with_url] input='{}', error={:?}",
                query, e
            );
...
impl<'js> UrlSearchParams<'js> {
    ...
    pub fn new(_ctx: Ctx<'js>, init: Opt<Value<'js>>) -> rquickjs::Result<Self> {
        let mut inner = ada_url::UrlSearchParams::parse("").map_err(|e| {
        ...
        if let Some(value) = init.0 {
            ...
            if let Some(query) = value.as_string() {
                ...
                inner = ada_url::UrlSearchParams::parse(&query_str).map_err(|e| {
                    eprintln!(
